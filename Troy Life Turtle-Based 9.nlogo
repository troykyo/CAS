...
breed [oldcells oldcell];;remain for a while but dont effect the pattern
breed [eternals eternal];;some things are forever
breed [stucks stuck];;some things are forever
turtles-own[counter PersonalColor living? shapeSetting]
globals [oldDensity currentDensity oldColor densitySpike]

patches-own [
  live-neighbors  ;; count of how many neighboring cells are alive
  dynamic? ;; can it change
]

to setup-blank
  clear-all
   set-default-shape cells "dot"
  set-default-shape babies "circle"
  set-default-shape eternals "lw"
    set-default-shape stucks "th"
  ask patches
...
to setup-random
  ...
  ask patches
    [ if random-float 100.0 < initial-density
      [ sprout-babies 1 ] ]
 ask eternals [set size 4
  set color brown]
  ask stucks [set size 3]
...
to go
 ...
 ask oldcells
  ...
  ask oldcells with [counter > 60]
   [ die ]
  ;; babies become alive
  ask babies
    [ set breed cells
      set PersonalColor TrendColor
      set color PersonalColor]
  ...
  ask eternals
      [ ask neighbors
      [ set live-neighbors live-neighbors + 2 ] ]
   ask stucks
      [ ask neighbors
      [ set live-neighbors live-neighbors - 1 ] ]
  ...
  ask cells
    [ ifelse live-neighbors = 2 or live-neighbors = 3
      [ set color PersonalColor ]
      [ set breed oldcells ;;promote to old
        set color PersonalColor ]]  ;; gray cells will die next round
                           ;; Now we handle the birth rule.
  ask patches
    [ if not any? cells-here and live-neighbors = 3 and dynamic? != false
      [ birth ]
   ...
   ask patches
  [if ticks  mod  14 = 5
    [set oldColor TrendColor
      set TrendColor ticks
     set oldDensity currentDensity
     set currentDensity count cells / count patches
    if oldDensity = currentDensity
       [if random-float 100.0 < .1
         [set densitySpike 5
          sprout-babies 2 ] ] ]
  ]
 ;; export-interface (word "results " date-and-time ".png")
  tick
  set densitySpike 0
end
;;////////////////////////////////////////////////////////////////////////////////
...
;; user adds a cell with the mouse
to draw-h  ;; patch procedure
  if not any? cells-here
    [ ask turtles-here [ die ]  ;; old cells and babies go away
      sprout-eternals 1 [ set color brown set size 4]]
...
;; user adds a cell with the mouse
to draw-n  ;; patch procedure
  if not any? cells-here
      [ ask turtles-here [ die ]  ;; old cells and babies go away
        sprout-stucks 1 [ set color white set size 3 ]]
...
to update  ;; patch procedure
  ...
  ifelse any? cells-here
    [ ifelse n = 2 or n = 3
      [ ask cells-here [ set color TrendColor] ]
...
262
491
How Trendy is the moment we live in?
(count cells\n/ count patches) * 1000
2
1
11
...
274
177
Press here to add your own Fashion Trend!
11
125.0
1
...
"" ""
PENS
"trend" 1.0 0 -16777216 true "" "plot count turtles / count patches * 10"
"pen-1" 1.0 2 -2674135 true "" "plot densitySpike"
...
293
248
These are people or places where fashion flourishes.  
11
125.0
1
...
294
324
These are people and places where fashion goes to die. 
11
64.0
1
...
false
0
Polygon -7500403 true true 120 15 120 0 0 0 0 15 30 15 120 270 150 270 270 15 300 15 300 0 180 0 180 15 255 15 150 225 75 15 120 15
Polygon -7500403 true true 30 285 180 285 240 180 255 180 210 300 105 300 15 300
Polygon -7500403 true true 165 60 165 60 90 285 60 285 135 60 165 60
Polygon -7500403 true true 150 60 195 60 210 45 120 45 105 60 135 60

lw
false
0
Polygon -7500403 true true 120 16 120 1 0 1 0 16 30 16 120 271 152 270 270 16 300 16 300 1 180 1 180 16 255 16 150 226 75 16 120 16
Polygon -7500403 true true 30 285 180 285 240 180 255 180 210 300 105 300 15 300
Polygon -7500403 true true 165 60 165 60 90 285 60 285 135 60 165 60
Polygon -7500403 true true 150 60 195 60 210 45 120 45 105 60 135 60
Polygon -7500403 true true 243 12 246 12 123 253 115 241 229 10 244 12 246 12
Polygon -7500403 true true 0 9 54 266 96 269 109 181 77 242 13 -1
...
false
0
Rectangle -1 true false 150 75 300 225
Rectangle -2674135 true false 0 75 150 225
Rectangle -13345367 true false 0 30 300 75
Rectangle -13345367 true false 0 225 300 270
